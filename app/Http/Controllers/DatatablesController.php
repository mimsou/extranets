...
class DatatablesController extends Controller
{
                               ...
                               $isCompletedChecked = false, $isHourlyChecked = false)
    {
        $projets = DB::table('projets')
            ->select(
                ['projets.*', 'employeurs.nom', 'demandes.projet_id', 'demandes.completed',
        ...
        if($personne != 'ALL' && $personne != null && $personne != 'undefined') {
            $projets->join('demande_users', 'demandes.id', '=', 'demande_users.demande_id')
                ->select(
                    ['projets.*', 'employeurs.nom', 'demandes.projet_id', 'demandes.id as projet_demande_id',
        ...
        if(\Auth::user() && \Auth::user()->role_lvl == 3) {
            ...
            $project_demande = Projet::select('id')->where('employeur_id', '<>', $user_employee_id)
                ->whereHas(
                    'demandes', function($q) use ($user_employee_id) {
        ...
        if($isCompletedChecked == false || $isCompletedChecked == 'false') {
            ...
            $projectIdsToRemove = [];
            $projectCloned->get()->groupBy('id')->map(
                function($item) use (&$projectIdsToRemove) {
...
//        $projets->distinct('projets.id');
        ...
        return Datatables::of($projets)
            ...
            ->addColumn(
                'employeur_name', function($m) {
                return $m->nom;
            })
            ->addColumn(
                'responsable', function($m) {
                $m = Projet::find($m->id);
                if(!is_null($m->responsable)) return $m->responsable->fullname;
                return 'NA';
            })
            ->addColumn(
                'statut_candidat', function($m) {
                return 0;
                // return count($m->candidats) .' / '. $m->nb_candidats;
            })
            ->addColumn(
                'childrow_html', function($m) use ($statut_du_dossier, $isCompletedChecked, $isHourlyChecked) {
                $m = Projet::find($m->id);
    ...
    public function getRegroupements()
    {
        return Datatables::of(Regroupement::query())
            ->editColumn(
                'updated_at', function(Regroupement $r) {
                return $r->updated_at->diffForHumans() . '<br><small>' . $r->updated_at . '</small>';
            })
            ->addColumn(
                'action', function(Regroupement $r) {
    ...
    public function getEmplois()
    {
        return Datatables::of(Emploi::query())
            ->editColumn(
                'updated_at', function(Emploi $r) {
                if(is_null($r->updated_at)) return '---';
                return $r->updated_at->diffForHumans() . '<br><small>' . $r->updated_at . '</small>';
            })
            ->addColumn(
                'action', function(Emploi $r) {
    ...
    public function getPays()
    {
        return Datatables::of(Pays::query())
            ->editColumn(
                'updated_at', function(Pays $r) {
                if(is_null($r->updated_at)) return '---';
                return $r->updated_at->diffForHumans() . '<br><small>' . $r->updated_at . '</small>';
            })
            ->addColumn(
                'action', function(Pays $r) {
    ...
    public function getEmployeurs()
    {
        if(Auth::user()->role_lvl == 2) {
            $groupId = User::with('group_assoc')->find(Auth::user()->id)->group_assoc;
            if($groupId != null) {
                $model = Employeur::with('regroupement')->with('pays')->where('regroupement_id', $groupId->group_id);
            } else {
                $model = Employeur::with('regroupement')->with('pays');
            }
        } else {
            $model = Employeur::with('regroupement')->with('pays');
        }
        return Datatables::of($model)
            ...
            ->addColumn(
                'regroupement', function(Employeur $c) {
                if(!is_null($c->regroupement)) return $c->regroupement->title;
                return ' -- ';
            })
            ->addColumn(
                'pays', function(Employeur $c) {
                if(!is_null($c->pays)) return $c->pays->title;
                return ' -- ';
            })
            ->editColumn(
                'updated_at', function(Employeur $c) {
                return $c->updated_at->diffForHumans() . '<br><small>' . $c->updated_at . '</small>';
            })
            ->addColumn(
                'action', function(Employeur $c) {
                ...
                if(auth()->user()->role_lvl == 2) $delete = '';
