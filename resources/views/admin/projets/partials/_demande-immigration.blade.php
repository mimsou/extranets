    ...
    <div class="card-body px-1 py-3">
                ...
                @if(Auth::user()->role_lvl > 3)
                    <div class="assignee">
                        ...
                        <div class="add-new-assignee-wrapper mt-2" style="display: none">
                            {!! Form::select('assign_user', \App\Models\User::whereIn('role_lvl', [10, 5])->get()->pluck('full_name', 'id'), null, ['placeholder'=>'Sélectionnez un utilisateur','class'=>'form-control select2 assign_demande', 'data-demande-id' => $p->id]) !!}
                        </div>
                    </div>
                @endif
            </div>
        ...
        <div class="time-progresssion mb-2">
            <div class="progression text-right" id="part_prog_{{ $p->id }}"
                 style="transition: all 0.5s ease; background-color: aquamarine; width:{{ demandeProgression($p->statut) }}; height:3px"></div>
        </div>
        ...
        @if(Auth::user()->role_lvl > 3)
            ...
            <div id="{{ $scope }}">
                <div class="row">
                    ...
                    <div class="col-md-12">
                        @include('admin.partials._comments',['demande'=>$p,'notes'=> $notes->take(2)->reverse()])
                    </div>
                </div>
            </div>
        @endif
